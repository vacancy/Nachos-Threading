...
public final class ElevatorGui extends Frame {
    ...
    private class ElevatorShaft extends Canvas {
        ...
        public void paint(Graphics g) {
            ...
            Rectangle d = new Rectangle(s * 3 / 2, h * (numFloors - 1 - floor), w - 3 * s, w / 3 - s);

            // unit of measurement in direction rect (12ux4u)
            int u = d.width / 12;

            // draw elevator, fill riders
            Rectangle e = new Rectangle(d.x, d.y + d.height, d.width, h - d.height - u);
            ...
            if (doorsOpen) {
                ...
                g.drawLine(e.x + e.width - 2 * s, e.y, e.x + e.width - 2 * s, e.y + e.height);
                for (int y = 0; y < e.height - 2 * s; y += 2 * s)
