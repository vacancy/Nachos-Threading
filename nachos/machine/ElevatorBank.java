...
public final class ElevatorBank implements Runnable {
     ...
     *
     * @param numElevators
     *            the number of elevators in the bank.
     * @param numFloors
     ...
     *            the rider to add.
     * @param floor
     *            the floor the rider will start on.
     * @param stops
     *            the array to pass to the rider's <tt>initialize()</tt> method.
    ...
    void postRiderEvent(int event, int floor, int elevator) {
        ...
        for (int i = 0; i < numRiders; i++) {
            ...
            if ((rider.inElevator && rider.elevator == e.elevator) || (!rider.inElevator && rider.floor == e.floor)) {
                rider.events.add(e);
                rider.schedule(1);
            }
        }
    ...
    private class ElevatorManager implements ElevatorControls {
        ...
        public void openDoors(int elevator) {
            Lib.assertTrue(elevator >= 0 && elevator < numElevators);
            postRiderEvent(RiderEvent.eventDoorsOpened, elevators[elevator].openDoors(), elevator);
        ...
        public void closeDoors(int elevator) {
            Lib.assertTrue(elevator >= 0 && elevator < numElevators);
            postRiderEvent(RiderEvent.eventDoorsClosed, elevators[elevator].closeDoors(), elevator);
        ...
        public void setDirectionDisplay(int elevator, int direction) {
            ...
            if (elevators[elevator].doorsOpen) {
                postRiderEvent(RiderEvent.eventDirectionChanged, elevators[elevator].floor, elevator);
            }
            ...
            for (int i = 0; i < numElevators; i++) {
                if (elevators[i].atNextFloor()) {
                    ...
                    if (elevators[i].atDestination()) {
                        postEvent(ElevatorEvent.eventElevatorArrived, elevators[i].destination, i, false);
                    } else {
                        elevators[i].nextETA += Stats.ElevatorTicks;
                        privilege.interrupt.schedule(Stats.ElevatorTicks, "elevator", interrupt);
                    }
                }
            }
    ...
    private class ElevatorState {
        ...
        boolean enter(RiderState rider, int onFloor) {
            ...
            if (!doorsOpen || moving || onFloor != floor || riders.size() == maxRiders)
                return false;
    ...
    private class RiderState implements RiderControls {
        ...
        public boolean pressUpButton() {
            ...
            for (int elevator = 0; elevator < numElevators; elevator++) {
                if (elevators[elevator].doorsOpen && elevators[elevator].direction == ElevatorBank.dirUp
                        && elevators[elevator].floor == floor)
                    return false;
            }

            manager.postEvent(ElevatorEvent.eventUpButtonPressed, floor, -1, true);
        ...
        public boolean pressDownButton() {
            ...
            for (int elevator = 0; elevator < numElevators; elevator++) {
                if (elevators[elevator].doorsOpen && elevators[elevator].direction == ElevatorBank.dirDown
                        && elevators[elevator].floor == floor)
                    return false;
            }

            manager.postEvent(ElevatorEvent.eventDownButtonPressed, floor, -1, true);
        ...
        public boolean enterElevator(int elevator) {
            Lib.assertTrue(!inElevator && elevator >= 0 && elevator < numElevators);
        ...
        public boolean pressFloorButton(int floor) {
            ...
            if (elevators[elevator].doorsOpen && elevators[elevator].floor == floor)
                return false;

            manager.postEvent(ElevatorEvent.eventFloorButtonPressed, floor, elevator, true);
