...
public class NetKernel extends VMKernel {
    ...
    private void ping(int dstLink) {
        ...
        try {
            ping = new MailMessage(dstLink, 1, Machine.networkLink().getLinkAddress(), 0, new byte[0]);
    ...
    private void pingServer() {
        while (true) {
            ...
            try {
                ack = new MailMessage(ping.packet.srcLink, ping.srcPort, ping.packet.dstLink, ping.dstPort,
                        ping.contents);
            } catch (MalformedPacketException e) {
                // should never happen...
                continue;
